...
## Phase 1 实施记录（本会话继续 - 2026-06-23）

### 本次会话新交付

| Task | 状态 | 提交 | 关键产物 | 测试数 |
|------|------|------|----------|--------|
| 1.5 LLMCatalog | ✅ | f62683b | `src/main/core/llmcalls/modelFactory.ts`（5 provider + 大小写不敏感 + 动态注册） | 11 |
| 1.6 Messages + Stream Format | ✅ | (commit) | `src/main/core/messages/{converter,trim}.ts` + `format/stream/{Base,Context,Default,Ollama}.ts` + index | 46 |
| 1.7 Tools (Phase 1 简化版) | ✅ | (commit) | `tools/{events,subagentRegistry,subagentMessageExtractor,stopSignal,BaseTools,FilesystemReadTools,base/BaseFilesystemTool}.ts` | 49 |
| 1.8 Router | ✅ | (commit) | `core/router/{fileUpload,fileDownload}Router.ts`（7 个 IPC channel） | 16 |
| 1.9 AgentDescriptor + RuntimeContext | ✅ | db9c955 | `src/shared/types/agentDescriptor.ts`（**spec §3.1 核心创新**）+ `applyConfigOverrides` | 29 |
| P1 收尾 | ✅ | (待 commit) | typecheck 0 / lint 0 / build OK / **211 测试通过** | — |

**测试总数**：42 (上会话) + 169 (本会话) = **211 通过 / 0 typecheck / 0 lint / build OK**

### 本次新增的关键决策（已落地）

1. **LLMCatalog 5 家 provider + 动态注册**：
   - 支持 `openai/anthropic/deepseek/google/ollama`（原 Python 4 家 + 新增 google）
   - `registerModelCreator(name, creator)` 支持运行时扩展新 provider
   - 大小写不敏感（`OpenAI` / `OPENAI` / `openai` 等价）
   - 未知 provider 抛 Error 含支持列表

2. **LangChain 1.x 适配**：
   - `trim_messages` 在 v1.x 已删除（v0.3 弃用）→ 自实现简化版 token-level trim
   - `ToolMessage({ content, tool_call_id })` 必须用 fields 形式构造，1.x 不再支持 `(content, id, name)` 两参
   - `content` 类型是 `$InferMessageContent` 联合类型，测试断言需 `String(...)` 兜底

3. **停止信号用 Node AsyncLocalStorage**：
   - Python 原版用 `contextvars.ContextVar`，Node 等价物是 `AsyncLocalStorage`
   - 跨 `await` 保持 context；多 IPC 调用并发隔离

4. **Tools Phase 1 简化版（Phase 5/6 接入真子智能体）**：
   - `BaseFilesystemTool` 当前实现 = 路径校验 + 目录清单占位 + tool_start/stop/error 事件
   - 真实 LangGraph 子智能体流式执行在 Phase 5 接入
   - `BaseTools` 只含 `get_current_time` + `echo_input`（Phase 1 验证用）
   - 复杂工具（`open_file` / `read_cached_chunk` / `load_web_page` / `sandbox` / `human_in_the_loop` / `mcp_*`）推迟到 Phase 5

5. **Router IPC channel 命名约定**：
   - `file:upload` / `file:upload-base64` / `file:list` / `file:delete`
   - `file:download-list` / `file:download-zip` / `file:download-read`
   - 真实实现（`FileService` / `FileDownloadService`）推迟到 Phase 4
   - Phase 1 只验证 IPC 契约（channel 名 + handler 转发）

6. **AgentDescriptor 核心创新落地**：
   - `AgentDescriptorSchema`（Zod）— 完整字段校验
   - `ToolRefSchema`（discriminatedUnion）— builtin/local/mcp 三种 kind
   - `MiddlewareRefSchema` — 中间件引用（Phase 5 实现中间件）
   - `RuntimeContextSchema` — 运行时 sessionId/profileId/abortSignal/metadata
   - `CommandRequestSchema` — `/command` 入参 + configOverrides
   - `CommandChunkSchema`（discriminatedUnion）— 9 种 type 流式事件
   - `applyConfigOverrides(base, overrides)` — 运行时覆盖 helper
   - `BUILTIN_AGENT_ID_PATTERN` — 小写 + kebab-case 命名约束

7. **Zod 4.4.3 API 变化**：
   - `z.string().min(1)` → 必须带 message 参数：`z.string().min(1, "must not be empty")`
   - `z.record(z.unknown())` → 必须带 key schema：`z.record(z.string(), z.unknown())`
   - ESLint globals 新增 `AbortController` / `AbortSignal`

### 工程指标（本会话后）

- **测试**: 211 通过（19 文件）/ coverage ≥70%
- **TypeScript**: 0 errors（strict + exactOptionalPropertyTypes + noImplicitAny 全开）
- **ESLint**: 0 errors
- **Build**: vue-tsc + vite build + tsc electron 全通过
- **Commits（本会话新增 5 个）**:
  - f62683b feat(llmcalls): LLMCatalog 5-provider factory + 11 tests
  - (commit) feat(core): messages/converter+trim + format/stream + 46 tests
  - (commit) feat(tools): events + subagent registry/extractor + stop signal + base/fs/sandbox-stubs + 49 tests
  - (commit) feat(router): file upload/download IPC handlers + 16 tests
  - db9c955 feat(shared): AgentDescriptor + RuntimeContext + CommandRequest/Chunk + 29 tests

### 下次会话快速恢复指引（已更新）

1. **目标工程**：`E:\laboratory\AI\Agents-desktop`
2. **环境**：`conda activate "E:\laboratory\AI\Agents"` + `pnpm@10.34.4`
3. **当前状态**：`pnpm test` → **211/211 通过**；`pnpm typecheck` 0 errors；`pnpm lint` 0 errors；`pnpm build` 成功
4. **下一步起点**：Phase 2 — MCPConfigSchema (Zod) + ConfigManager + UnifiedMCPClient + McpConfigView.vue
5. **参考文档**：
